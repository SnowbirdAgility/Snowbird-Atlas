/* Snowbird Atlas — landing interactions */
(function(){
  ...
  ids.forEach(id=>{ const el=document.getElementById(id); if(el) secObserver.observe(el); });
  ...
  document.querySelectorAll('.reveal').forEach((el,i)=>{
    // small stagger for grid items sharing a parent
  ...
  function openModal(reason){
    ...
    if(reason){
      [...reasonSel.options].forEach(o=>{ if(o.text.toLowerCase().includes(reason.toLowerCase())) reasonSel.value = o.value; });
    }
  ...
  document.querySelectorAll('[data-open-modal]').forEach(b=>{
    b.addEventListener('click', (e)=>{ e.preventDefault(); openModal(b.getAttribute('data-reason')); setMenu(false); });
  });
  ...
  ['f-name','f-email','f-org'].forEach(id=>{
